...
class AscendAttentionCPImpl(AscendAttentionBackendImpl):
    ...
    def ring_attn(self, query, key, value, attn_metadata):
        ...
        for i in range(pcp_size):
            ...
            cur_half_kv = current_k.shape[0] // 2
            ...
            if s == pcp_rank:
                output_i, lse_i = torch.ops.npu.npu_fused_infer_attention_score(
                    ...
                    antiquant_scale=None,
                    softmax_lse_flag=True,
                    actual_seq_lengths_kv=[current_k.shape[0]] if attn_metadata.prefill else None,
                    actual_seq_lengths=[query.shape[0]] if attn_metadata.prefill else None,
                )

            elif s < pcp_rank:
                kv_head_k = current_k[:cur_half_kv]
                kv_head_v = current_v[:cur_half_kv]

                output_i, lse_i = torch.ops.npu.npu_fused_infer_attention_score(
                    query, kv_head_k, kv_head_v,
                    num_heads=self.num_heads,
                    num_key_value_heads=self.num_kv_heads,
                    input_layout="TND",
                    atten_mask=None,
                    ...
                    antiquant_scale=None,
                    softmax_lse_flag=True,
                    actual_seq_lengths_kv=[cur_half_kv] if attn_metadata.prefill else None,
                    actual_seq_lengths=[query.shape[0]] if attn_metadata.prefill else None,
                )

            else:
                q_tail = query[half_kv:].contiguous()

                out_tail, lse_tail = torch.ops.npu.npu_fused_infer_attention_score(
                    q_tail, current_k, current_v,
                    num_heads=self.num_heads,
                    num_key_value_heads=self.num_kv_heads,
                    input_layout="TND",
                    atten_mask=None,
                    ...
                    softmax_lse_flag=True,
                    actual_seq_lengths_kv=[current_k.shape[0]] if attn_metadata.prefill else None,
                    actual_seq_lengths=[q_tail.shape[0]] if attn_metadata.prefill else None,
                )
        ...
        if tail_outputs:
            ...
            out_head = torch.zeros(half_kv, query.shape[1], self.head_size,
                                   dtype=query.dtype, device=query.device)
            lse_head = torch.full((half_kv, query.shape[1], 1), float('-inf'),
                                  dtype=merged_tail_lse.dtype, device=query.device)
            output_i = torch.cat([out_head, merged_tail_out], dim=0)
            lse_i = torch.cat([lse_head, merged_tail_lse], dim=0)
    ...
    def forward_impl(
        ...
        if has_prefill:
            ...
            if self.pcp_size == 1:
                ...
                # zigzag 切分：前半段顺序，后半段倒序
                # rank r: head=[kv_base + r*half_kv : kv_base + (r+1)*half_kv]
                #         tail=[kv_base + total - (r+1)*half_kv : kv_base + total - r*half_kv]
