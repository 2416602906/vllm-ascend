...
class AscendAttentionCPImpl(AscendAttentionBackendImpl):
    ...
    def _forward_decode_pcp_dcp(self, query: torch.Tensor, attn_metadata: AscendMetadata) -> torch.Tensor:
        ...
        # Calculate local KV lengths on the fly for interleaved sharding
        # total_len = seq_lens, pcp_size = self.pcp_size
        total_seq_lens = attn_metadata.seq_lens
        actual_seq_lengths_kv = (total_seq_lens // self.pcp_size) + (self.pcp_rank < (total_seq_lens % self.pcp_size)).int()
        actual_seq_lengths_kv = actual_seq_lengths_kv.to(torch.int32)
        
        # Ensure k_nope/value match FIA expected layout BnNBsD: [num_blocks, num_kv_heads, block_size, head_size]
        # self.key_cache is [num_blocks, block_size, num_kv_heads, head_size]
        # We must slice the tensor to match the compacted block_size for the operator's shape check
        compact_block_size = self.key_cache.shape[1] // self.pcp_size
        k_nope = self.key_cache[:, :compact_block_size, :, :].transpose(1, 2)
        value = self.value_cache[:, :compact_block_size, :, :].transpose(1, 2)
        ...
        common_kwargs = {
            ...
            # CRITICAL: Use compacted block size to match local sharded storage
            "block_size": self.key_cache.shape[1] // self.pcp_size,
            "actual_seq_lengths_kv": actual_seq_lengths_kv,
            "actual_seq_lengths": torch.arange(attn_metadata.num_decodes_flatten, device=query.device, dtype=torch.int32) + 1,
        }
        ...
        if _EXTRA_CTX.capturing:
            ...
            graph_params.attn_params[num_tokens].append(
                (
                    ...
                    common_kwargs["block_size"],
                    actual_seq_lengths_kv,
                    common_kwargs["actual_seq_lengths"],
        ...
        # Robust global merge using dedicated NPU operators
    ...
    def reshape_and_cache(
        ...
        if len(kv_cache) > 1:
            ...
            if has_decode:
                # Filter and COMPACT slots for local sharded storage
                slot_mapping = attn_metadata.slot_mapping[self.pcp_rank : num_decode_tokens * self.pcp_size : self.pcp_size]
                slot_mapping = torch.div(slot_mapping, self.pcp_size, rounding_mode='floor')
            ...
            if has_prefill:
                if self.pcp_size > 1:
                    ...
                    # Keep the local QKV as-is for Ring-Attention
                    self.local_prefill_key = key
                    self.local_prefill_value = value
                    
                    # Filter and COMPACT slots [rank::pcp_size] for local storage.
                    # This ensures tokens are stored contiguously in each rank's local cache.
                    start_idx = self.pcp_size * num_decode_tokens
                    end_idx = attn_metadata.num_actual_tokens_pcp_padded
                    slot_mapping = attn_metadata.slot_mapping[start_idx + self.pcp_rank : end_idx : self.pcp_size]
                    slot_mapping = torch.div(slot_mapping, self.pcp_size, rounding_mode='floor')
    ...
    def forward_impl(
        ...
        if has_prefill:
            ...
            if self.pcp_size == 1:
                ...
                # Skip decode tokens if present at the beginning of the local tensors
                key = self.local_prefill_key[num_decode_tokens:].contiguous()
                value = self.local_prefill_value[num_decode_tokens:].contiguous()
