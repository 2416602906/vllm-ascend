...
class AscendAttentionCPImpl(AscendAttentionBackendImpl):
    ...
    def ring_attn(self, query, key, value, attn_metadata):
        ...
        # 1. Sync once to get metadata for the loop and sequence lengths
        # Use safe conversion to handle both Tensor and list
        ...
        # 2. Optimization: Reconstruct interleaved indices directly on NPU
        # This guarantees bit-wise accuracy with the original interleaved layout
        # while eliminating CPU-side allocation and H2D transfer overhead.
        local_head_indices = []
        local_tail_indices = []
        start = 0
        for i in range(len(half_seqlens)):
            h_len = half_seqlens[i] if i == 0 else half_seqlens[i] - half_seqlens[i-1]
            local_head_indices.append(torch.arange(start, start + h_len, device=device))
            local_tail_indices.append(torch.arange(start + h_len, start + 2 * h_len, device=device))
            start += 2 * h_len
        
        local_head_idx = torch.cat(local_head_indices)
        local_tail_idx = torch.cat(local_tail_indices)
        
        # 3. Optimization: Pre-select loop-invariant q_tail once outside the loop
