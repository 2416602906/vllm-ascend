...
class AscendAttentionCPImpl(AscendAttentionBackendImpl):
    def __init__(
        ...
        **kwargs,
        ...
        self.ring_send_stream = torch.npu.Stream()
        self.ring_recv_stream = torch.npu.Stream()
    ...
    def reshape_and_cache(
        ...
        if len(kv_cache) > 1:
            ...
            if has_prefill:
                ...
                if self.pcp_size > 1 and not attn_metadata.prefill.pcp_metadata.pcp_use_hybrid_attn:
                    # Ring-Attn: Only cache local KV
                    prefill_key = key[num_decode_tokens:]
                    prefill_value = value[num_decode_tokens:]

                    total_prefill_tokens = attn_metadata.num_actual_tokens_pcp_padded - self.pcp_size * num_decode_tokens
                    tokens_per_rank = total_prefill_tokens // self.pcp_size
                    start_idx = self.pcp_size * num_decode_tokens + self.pcp_rank * tokens_per_rank
    ...
    def ring_attn(self, query, key, value, attn_metadata):
        ...
        for i in range(pcp_size):
            # 1. Prepare current KV data
            current_k, current_v = kv.split([self.head_size, self.head_size], dim=-1)
            current_s = (pcp_rank - i + pcp_size) % pcp_size

            # 2. Launch FIA Calculation (Async on NPU)
            # Launching FIA first ensures the compute engine is occupied immediately
            if current_s == pcp_rank:
                # Local Round
            ...
            # 3. Start Communication for next round AFTER current round's FIA is launched
            # Use independent streams and interleaved order to avoid HCCL tag/cmdType inconsistency.
            if i < pcp_size - 1:
                if pcp_rank % 2 == 0:
                    # Even ranks: Send then Recv
                    with torch_npu.npu.stream(self.ring_send_stream):
                        send_req = torch.distributed.isend(kv, send_rank, group=device_group)
                    with torch_npu.npu.stream(self.ring_recv_stream):
                ...
                # 4. Wait for recv to finish before starting the next iteration.
                # We only need to wait for recv_stream because recv_buff must be ready for next round's FIA.
                # The send_stream does not block the next round's computation as it's read-only for kv.
                torch.npu.current_stream().wait_stream(self.ring_recv_stream)
