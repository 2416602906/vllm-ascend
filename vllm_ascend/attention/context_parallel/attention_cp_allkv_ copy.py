...
class AscendAttentionCPImpl(AscendAttentionBackendImpl):
    ...
    def ring_attn(self, query, key, value, attn_metadata):
        ...
        # Since key/value/query are already organized as [All_Heads, All_Tails]
        # we can use simple slicing to avoid index_select overhead.
        split = half_seqlens[-1]
        ...
        for i in range(pcp_size):
            ...
            if s == pcp_rank:
                # Optimized Local Rank: Two masks, one nomask
                # 1. q_head vs k_head (Mask)
                out_h_h, lse_h_h = torch.ops.npu.npu_fused_infer_attention_score(
                    query[:split],
                    current_k[:split],
                    current_v[:split],
                    num_heads=self.num_heads,
                ...
                # 2. q_tail vs k_head (No Mask)
                out_t_h, lse_t_h = torch.ops.npu.npu_fused_infer_attention_score(
                    query[split:],
                    current_k[:split],
                    current_v[:split],
                    num_heads=self.num_heads,
                ...
                # 3. q_tail vs k_tail (Mask)
                out_t_t, lse_t_t = torch.ops.npu.npu_fused_infer_attention_score(
                    query[split:],
                    current_k[split:],
                    current_v[split:],
                    num_heads=self.num_heads,
                ...
                # Merge tail results (Tail-Head nomask and Tail-Tail mask)
                out_tail, lse_tail = torch_npu.npu_attention_update(
                    [lse_t_h.flatten(1, -1), lse_t_t.flatten(1, -1)],
                    [out_t_h.flatten(1, 2), out_t_t.flatten(1, 2)], 0)
                out_tail = out_tail.view(out_t_h.shape)
                lse_tail = lse_tail.view(lse_t_h.shape)

                output_i = torch.empty_like(query)
                lse_i = torch.empty((query.shape[0], query.shape[1], 1),
                                   dtype=lse_h_h.dtype,
                                   device=query.device)

                output_i[:split] = out_h_h.to(query.dtype)
                lse_i[:split] = lse_h_h
                output_i[split:] = out_tail.to(query.dtype)
                lse_i[split:] = lse_tail
                ...
                tail_outputs.append(out_tail.flatten(1, 2))
                tail_lses.append(lse_tail.flatten(1, -1))
        ...
        if tail_outputs:
            merged_tail_out, merged_tail_lse = torch_npu.npu_attention_update(tail_lses, tail_outputs, 0)
            merged_tail_out = merged_tail_out.view(-1, self.num_heads, self.head_size)
            merged_tail_lse = merged_tail_lse.view(-1, self.num_heads, 1)
            ...
            lse_i = torch.full((query.shape[0], query.shape[1], 1),
                               ...
                               device=query.device)
            output_i[split:] = merged_tail_out.to(query.dtype)
            lse_i[split:] = merged_tail_lse
            outputs_list.append(output_i)
            lses_list.append(lse_i)
        ...
        # Final merge of all ranks' contributions
        # Use npu_attention_update to avoid torch.stack and manual math overhead.
        output, lse = torch_npu.npu_attention_update(
            [lse.flatten(1, -1) for lse in lses_list],
            [out.flatten(1, 2) for out in outputs_list], 0)
        
        output = output.view(query.shape[0], self.num_heads, self.head_size)
        lse = lse.view(query.shape[0], self.num_heads, 1)
