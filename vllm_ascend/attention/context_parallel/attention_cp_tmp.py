...
class AscendAttentionCPImpl(AscendAttentionBackendImpl):
    def __init__(
        ...
        **kwargs,
        ...
        self.ring_comm_stream = torch.npu.Stream()
    ...
    def ring_attn(self, query, key, value, attn_metadata):
        ...
        for i in range(pcp_size):
            ...
            # Using a separate stream ensures communication overlaps with FIA computation.
            if i < pcp_size - 1:
                with torch_npu.npu.stream(self.ring_comm_stream):
                    if pcp_rank % 2 == 0:
                ...
                # 4. Wait for communication to finish on NPU before starting the next iteration.
                # The CPU does not block here, allowing it to enqueue the next round's FIA immediately.
                # The NPU default stream will wait for the ring_comm_stream to finish the P2P transfer.
                torch.npu.current_stream().wait_stream(self.ring_comm_stream)
